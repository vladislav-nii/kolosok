...
async function test(number) {
    //window.location.href = `http://localhost:5500/surveys/survey${number}`;
    category_id = document.cookie.replace(/(?:(?:^|.*;\s*)category_id\s*\=\s*([^;]*).*$)|^.*$/, "$1");
    console.log(category_id);
    window.location.href = `http://localhost:5500/categories/category${category_id}/survey${number}`;
}

resultsBtn.addEventListener('click', () => {
    location.assign("http://localhost:5500/user-results");  
})

backBtn.addEventListener('click', () => {
    location.assign("http://localhost:5500/categories");  
})

async function applyStyle(){
    const response = await fetch('http://localhost:5500/is-available');
    const response2 = await fetch('http://localhost:5500/opening-time');
